...
`pip install mutiny-ai` (CLI stays `mutiny`; see root README /
[PUBLISHING.md](./PUBLISHING.md)). Sample/demo agents are **reference harnesses**,
...
| [HOSTED_INGESTION.md](./HOSTED_INGESTION.md) | CLI → Hosted ingest contract + API + CLI sync + Web observe + exec removal (M-PR8A–E) | Optional `mutiny test --hosted` / `mutiny sync` |
...
**Install claims:** Prefer the root README. Primary path is `pip install mutiny-ai` (CLI stays `mutiny`). Never recommend bare `pip install mutiny` / `mutiny-sdk` (those names are taken by other projects). Git/source install is optional — see [PUBLISHING.md](./PUBLISHING.md).
