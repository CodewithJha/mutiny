...
**Install today:** from this repository with `uv` (not on PyPI yet). PyPI `pip install mutiny` is planned — do not treat it as available.
...
**When PyPI exists (planned — not available yet):**

```bash
# pip install mutiny   # roadmap only
