...
Primary: developers building AI agents who install Mutiny locally (`pip install mutiny` → `mutiny init` → `mutiny run`) to fuzz tool-use policies and freeze failures as regressions. Hackathon MVP assumption: their project uses the **OpenAI Agents SDK** (first adapter).

Secondary: AI/security engineers replaying regressions in CI; hackathon/demo audiences viewing Hosted lineage against a **sample** agent project (bundled demo as reference harness).
...
The Hackathon MVP ships with support for OpenAI Agents SDK projects through the first adapter.

Success (MVP): a developer with an OpenAI Agents SDK project can init Mutiny, connect via adapter, run a campaign, and get a verified violation → minimized regression — optionally inspect lineage in Hosted UI.
...
- Connect a local agent via adapter (MVP: OpenAI Agents SDK)
...
- Hackathon MVP: first production-quality adapter = **OpenAI Agents SDK** (other frameworks = roadmap adapters)
...
- How Hosted UI attaches to a customer-local `mutiny run` vs interim demo harness
